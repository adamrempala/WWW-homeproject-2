...
const sameQuizTwoTimes = () => {
    return it ('We cannot get into the quiz second time', async () => {
...
const logoutAfterChangingPassword = () => {
    return it ('After password change another sessions should log out', async () => {
        const driver = new Builder().forBrowser("firefox").build();
        await driver.get("http://localhost:3000");
        await driver.findElement(By.name("username")).sendKeys("user1");
        await driver.findElement(By.name("password")).sendKeys("user1");
...
const appBehavior = async () => {
    return it ('Returned JSON files should have proper structure', async () => {
        const driver = new Builder().forBrowser("firefox").build();
